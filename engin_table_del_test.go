...
func TestDeleteByPrimaryKey(t *testing.T) {
	...
	num, err := engine.TableDelete(User{}).ByPrimaryKey(1)
	as.Nil(err)
...
func TestDeleteByPrimaryKeys(t *testing.T) {
	...
	num, err := engine.TableDelete(User{}).ByPrimaryKey(1, 2, 3)
	as.Nil(err)
...
func TestDeleteByModel(t *testing.T) {
	...
	num, err := engine.TableDelete(User{}).ByModel(Whe{
...
func TestDeleteByWhere(t *testing.T) {
	...
	num, err := engine.TableDelete(User{}).ByWhere(new(WhereBuilder).
		Eq("name", "kk").
		Like("age", "kk"),
	)
	as.Nil(err)
